...
class SARC_Preprocessor():
    ...
    def prepare_vocabulary(self):
        ...
        for curr_response in self.response_list:
            for comment in curr_response:
                for word in nltk.word_tokenize(comment):
                    self.vocab[word] += 1

class SARC_GLOVE_Preprocessor(SARC_Preprocessor):
    def __init__(self, embedding_dim = 100,*args, **kwargs):
        super(SARC_Glove_Preprocessor, self).__init__()
    ...
    def build_dataset(self, dataset_size = 1e4, phase = 'train'):
        ...
        for curr_response in self.response_list:
            curr_response_feature_vector = []
            for comment in curr_response:
                words = nltk.word_tokenize(comment)
                words_embedding_avg = np.average(np.array([self.embedding_lookup_dict[w] if w in self.embedding_lookup_dict else unk for w in words]),
                                                 axis = 0)
                curr_response_feature_vector.append(words_embedding_avg)
            X.append(np.concatenate(curr_response_feature_vector, axis = 0))
...
class SARC_ELMO_Preprocessor(SARC_Preprocessor):
    def __init__(self, *args, **kwargs):
        super(SARC_ELMO_Preprocessor, self).__init__()
...
if __name__ == '__main__':
    ...
    preproc = SARC_ELMO_Preprocessor(embedding_dim=50)
    Xtr,Ytr = preproc.build_dataset(dataset_size=3e4, phase = 'test')
    print("X : {}".format(Xtr.shape))
    print("Y : {}".format(Ytr.shape))
    torch.save(Xtr,os.path.join(data_dir,"Xvalid_{}_preprocessed.pt".format(Xtr.shape[0])))
    torch.save(Ytr,os.path.join(data_dir,"Yvalid_{}_preprocessed.pt".format(Ytr.shape[0])))
